...
void t_expr_check(symbol_table *symt, t_expr *expr)
{
	...
	if (expr->type == 2) {
		...
		expr->num_ptr = expr->binop->lhs->num_ptr > expr->binop->rhs->num_ptr ? expr->binop->lhs->num_ptr : expr->binop->rhs->num_ptr;
...
void t_expr_set_typeinfo(symbol_table *symt, t_expr *leaf)
{
	if (leaf->type == 1) {
		...
		symbol *sym = symbol_table_lookup(symt, get_decl_name(leaf->decl_spec));
		leaf->num_ptr += sym->type.num_ptr;
		leaf->type_name = sym->type.type_name;
		//print_expr_type(leaf);
	}
...
void print_expr_type(t_expr *expr)
{
	...
	if (expr->type==0) printf("name: %s ", expr->ident->ident);
