...
void stmt_gen(quad_gen *gen, t_stmt *stmt)
{
	...
	switch (stmt->type) {
		...
		case 3://if/else statements
			...
			if (stmt->cstmt->condition->type != 2 && stmt->cstmt->condition->type != 4) {
				t_expr *expr = t_expr_init2(stmt->cstmt->condition, oper_notequal, t_expr_init1(t_numeric_init0("0")));
		...
		case 4://for/while loops
			break;
		default: break;//Dont add declarations to quads
	}
}
...
quad_operand *quad_opr_from_expr(quad_gen *gen, t_expr *expr)
{
	...
	if (expr->virt_reg != -1) {
		...
		opr->type = Q_SYM;
		opr->sym = symbol_table_lookup(gen->symt, get_decl_name(expr->decl_spec));
	} else if (expr->type == 4 && expr->unop->op == oper_deref) {
		opr->indirect = 1;
		if (expr->unop->term->type == 0) {
			opr->type = Q_SYM;
			opr->sym = symbol_table_lookup(gen->symt, expr->unop->term->ident->ident);
		}
		...
		printf("%d\n", expr->type);
		printf("FATAL ERROR: quad_opr_from_expr recieved unparsed interior node\n");
		abort();
		exit(1);
	}
...
void quad_from_binop(quad_gen *gen, t_expr *expr)
{
	...
	lptr_op = lptr_op && (expr->binop->op == oper_add || expr->binop->op == oper_sub);

	int rptr_op = expr->binop->rhs->num_ptr > 0;
	rptr_op = rptr_op && (expr->binop->lhs->num_ptr == 0);
	rptr_op = rptr_op && (expr->binop->op == oper_add || expr->binop->op == oper_sub);
	int size = PTR_SIZE;
	...
	if (op == quad_assign) {
		...
		if (lptr_op) {
			t_expr * ebop;
			ebop = t_expr_init2(expr->binop->rhs,oper_mult, t_expr_init1(t_numeric_init2(size)));
			...
			t_expr * ebop;
			ebop = t_expr_init2(expr->binop->lhs,oper_mult, t_expr_init1(t_numeric_init2(size)));
			quad_from_binop(gen, ebop);
			ebop->binop->rhs = NULL;
			t_expr_destroy(ebop);
