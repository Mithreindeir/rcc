...
void quad_list_replace(quad_list *list, int oldv, int newv)
{
    ...
    for (int i = 0; i < list->num_quads; i++) {
        if (list->quad_list[i] >= oldv) {
            list->quad_list[i] += diff;
        }
    }
}
...
int quad_list_min(quad_list *list)
{
    int min = INT_MAX;
    for (int i = 0; i < list->num_quads; i++) {
        min = list->quad_list[i] < min ? list->quad_list[i] : min;
    }
    return min;
}
...
int quad_list_max(quad_list *list)
{
    int max = INT_MIN;
    for (int i = 0; i < list->num_quads; i++) {
        max = list->quad_list[i] > max ? list->quad_list[i] : max;
    }
    return max;
}
...
{
    for (int i = 0; i < list->num_quads; i++) {
        gen->quads[list->quad_list[i]]->label = label;
    }
    list_destroy(list);
}
...
quad_list *merge(quad_list *l1, quad_list *l2)
{
    ...
    l1->quad_list = realloc(l1->quad_list, sizeof(int) * l1->num_quads);
...
quad_list *make_list(int idx)
{
    ...
    list->quad_list = malloc(sizeof(int));
    list->num_quads = 1;
    list->quad_list[0] = idx;
